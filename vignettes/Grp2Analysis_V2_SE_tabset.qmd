...
if (is.null(se_file) || !nzchar(se_file)) {
  se_file <- file.path(tempdir(), "prolfquapp-example-se.rds")
...
reader <- prolfquapp::DEAResultReader$new(se_file)
meta <- reader$metadata
lfq_raw <- reader$lfq_raw
lfq_transformed <- reader$lfq_transformed
contrast_table <- reader$contrast_table
# Column roles of the modelling backend that produced this result. Reading the
# names from here keeps the report free of backend-specific column names.
contrast_cfg <- reader$contrast_config
has_pvalue <- contrast_cfg$has_pvalue()
has_roles <- reader$has_roles
...
significant_sets_by_direction <- function(significant_contrasts, direction = c("all", "increased", "decreased")) {
  ...
  contrast_col <- contrast_cfg$contrast_col
  effect_col <- contrast_cfg$effect_col
...
settings <- prolfquapp:::.processing_options_from_list(
  meta$processing_options
)$parameters_table(
  software = meta$provenance$software,
  model = meta$default_model
)
if (!is.null(meta$formula$formula)) {
  settings <- rbind(
    settings,
    data.frame(
      Parameter = "Model formula",
      Value = as.character(meta$formula$formula[[1]])
    )
  )
}
...
if (has_roles(contrast_cfg$contrast_col)) {
  contrast_col <- contrast_cfg$contrast_col
...
if (!is.null(contrast_plotter)) {
  ...
  score_col <- if (has_pvalue && has_roles(contrast_cfg$pvalue_col)) {
    contrast_cfg$pvalue_col
  } else if (has_roles(contrast_cfg$fdr_col)) {
    contrast_cfg$fdr_col
  } else {
    NA_character_
  }
