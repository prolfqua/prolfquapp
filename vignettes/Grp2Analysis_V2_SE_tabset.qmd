...
if (is.null(artifact_file) || !nzchar(artifact_file)) {
  artifact_file <- file.path(tempdir(), "prolfquapp-example-se.rds")
  if (!file.exists(artifact_file)) {
    example_dea <- prolfquapp::example_deanalyse()
    example_cfg <- example_dea$prolfq_app_config
    example_cfg$path <- tempfile()
    dir.create(example_cfg$path, recursive = TRUE, showWarnings = FALSE)
    saveRDS(
      prolfquapp::DEAReportGenerator$new(
        example_dea,
        example_cfg
...
# Everything this report shows comes off the reader: the abundances, the
# contrast table under the column names the modelling backend produced, and
# the column roles that say which of those names to use.
reader <- prolfquapp::DEAResultReader$new(artifact_file)
...
significant_sets_by_direction <- function(significant_contrasts, direction = c("all", "increased", "decreased")) {
  ...
  feature_cols <- reader$subject_id
  contrast_col <- reader$contrast_config$contrast_col
  effect_col <- reader$contrast_config$effect_col
  ...
  feature_ids <- tidyr::unite(
    data[feature_cols],
    "feature",
    dplyr::everything(),
    sep = "~"
  )$feature
  sets <- split(feature_ids, data[[contrast_col]])
...
#| results: asis
cat(prolfquapp:::.report_overview_cards(reader$lfq_raw))
```
...
Input: quantified feature abundances from `r nrow(reader$lfq_raw$factors())` samples. The detailed source-data reference is recorded in **Session Info**.
...
settings <- prolfquapp:::.processing_options_from_list(
  reader$metadata$processing_options
)$parameters_table(
  software = reader$metadata$provenance$software,
  model = reader$metadata$default_model
)
if (!is.null(reader$metadata$formula$formula)) {
  settings <- rbind(
    settings,
    data.frame(
      Parameter = "Model formula",
      Value = as.character(reader$metadata$formula$formula[[1]])
    )
  )
}
dt_table(settings, caption = "Analysis settings", page_length = 15)
```
...
sample_counts <- as.data.frame(table(reader$lfq_raw$factors()[reader$lfq_raw$relevant_factor_keys()]))
...
sample_annotation <- reader$lfq_raw$factors()
...
contrast_definitions <- contrast_definition_table(reader$metadata$contrasts)
...
protein_counts <- safe_plot(
  reader$lfq_raw$get_Summariser()$plot_hierarchy_counts_sample(nr_children = 1),
...
#| fig-height: 6
draw_or_empty(
  reader$lfq_raw$get_Summariser()$upset_interaction_missing_stats(tr = 1),
...
#| fig-height: 6
show_plot(safe_plot(
  reader$lfq_raw$get_Plotter()$na_heatmap(),
...
missing_per_group <- safe_plot(
  reader$lfq_raw$get_Summariser()$plot_missingness_per_group(),
  "Could not draw missingness per group."
)
missing_cumsum <- safe_plot(
  reader$lfq_raw$get_Summariser()$plot_missingness_per_group_cumsum(),
  "Could not draw cumulative missingness per group."
)
missing_hist <- safe_plot(
  reader$lfq_raw$get_Plotter()$missigness_histogram(),
...
raw_density <- safe_plot(
  reader$lfq_raw$get_Plotter()$intensity_distribution_density(),
  "Could not draw raw abundance density."
) +
  ggplot2::labs(tag = "A")
trans_density <- safe_plot(
  reader$lfq_transformed$get_Plotter()$intensity_distribution_density(),
...
raw_violin <- safe_plot(
  reader$lfq_raw$get_Stats()$violin(),
  "Could not draw raw CV violin plot."
) +
  ggplot2::labs(y = "CV")
log2_raw <- reader$lfq_raw$get_Transformer()$log2()$lfq
...
trans_violin <- safe_plot(
  reader$lfq_transformed$get_Stats()$violin(),
...
variance_summary <- dplyr::bind_rows(
  raw_cv = reader$lfq_raw$get_Stats()$stats_quantiles()$wide,
  log2_sd = log2_raw$get_Stats()$stats_quantiles()$wide,
  transformed_sd = reader$lfq_transformed$get_Stats()$stats_quantiles()$wide,
  .id = "source"
) |>
...
#| fig-height: 6
pca_plot <- safe_plot(
  reader$lfq_transformed$get_Plotter()$pca(),
  "Could not draw PCA."
)
...
#| fig-height: 6
show_plot(safe_plot(
  reader$lfq_transformed$get_Plotter()$heatmap_cor(),
...
#| fig-height: 6
show_plot(safe_plot(
  reader$lfq_transformed$get_Plotter()$heatmap(),
...
if (reader$has_roles(reader$contrast_config$contrast_col)) {
  contrast_col <- reader$contrast_config$contrast_col
  abundance_summary <- reader$contrast_table |>
    dplyr::count(.data[[contrast_col]], name = "tested") |>
    dplyr::left_join(
      dplyr::count(significant_contrasts, .data[[contrast_col]], name = "significant"),
      by = contrast_col
    ) |>
    dplyr::mutate(
      significant = dplyr::coalesce(.data$significant, 0L),
      not_significant = .data$tested - .data$significant
    )
} else {
...
if (!is.null(contrast_plotter)) {
  # The plotter names its volcano panels after the score column it was given,
  # so the FDR panel is keyed by the backend's own FDR column (`BFDR` for
  # SAINT), never by the literal name "FDR".
  volcano_plot <- safe_plot(
    contrast_plotter$volcano()[[reader$contrast_config$fdr_col]],
    "Could not draw volcano plot."
  )
...
#| fig-height: 8
if (nrow(significant_contrasts) > 0) {
  significant_lfq <- reader$lfq_transformed$get_copy()
  significant_lfq <- significant_lfq$get_subset(significant_contrasts)
  row_names <- tryCatch(nrow(significant_lfq$hierarchy()) <= 30, error = function(e) FALSE)
...
if (!is.null(contrast_plotter)) {
  ...
  # Prefer the raw p-value when the backend produces one, otherwise its FDR.
  score_col <- if (reader$contrast_config$has_pvalue() && reader$has_roles(reader$contrast_config$pvalue_col)) {
    reader$contrast_config$pvalue_col
  } else if (reader$has_roles(reader$contrast_config$fdr_col)) {
    reader$contrast_config$fdr_col
  } else {
    NA_character_
  }
...
dt_table(reader$contrast_table, caption = "Differential abundance results", page_length = 25)
```
...
```{r}
#| label: tbl-bfabric
urls <- reader$metadata$bfabric_urls
