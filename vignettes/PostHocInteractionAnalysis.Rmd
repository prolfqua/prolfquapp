...
df <- inner_join(diffDB, diffDA , by = c("protein_Id", "IDcolumn", "description", "contrast"), suffix = c(".B",".A"))
```
...
df <- df |> mutate(pValue_I = 2*pt(q = abs(df$tstatistic_I), df = df$df_I, lower.tail = FALSE))
df <- df |> group_by(contrast) |> mutate(FDR = p.adjust(pValue_I, method = "BH"))
```
...
```{r}
df <- df |> mutate(sig = case_when(FDR < 0.1 & abs(diffA_diffB) > 1 ~ "sig", TRUE ~ "none"))
...
```{r}
df <- df |> ungroup() 
bb <- df |> select(protein_Id, starts_with("diff"), starts_with("FDR"))
...
ddf <- df |> select(protein_Id, starts_with("diff")) |> pivot_longer(cols=starts_with("diff"), names_to = "contrast", values_to = "diff")
ddf <- ddf |> mutate(contrast = case_when(contrast == "diff.B" ~ "B", contrast == "diff.A" ~ "A", TRUE ~ "AvsB"))

fdf <- df |> select(protein_Id, starts_with("FDR")) |> pivot_longer(cols=starts_with("FDR"), names_to = "contrast", values_to = "FDR")
fdf$contrast |> unique()
fdf <- fdf |> mutate(contrast = case_when(contrast == "FDR.B" ~ "B", contrast == "FDR.A" ~ "A", TRUE ~ "AvsB"))

an <- df |> select(protein_Id, sig)
xdf <- inner_join(ddf,fdf)
xdf <- inner_join(an, xdf)
