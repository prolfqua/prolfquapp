...
# The introduction below describes a difference-based test, so it only applies
# to backends that actually produce one. Ask the backend, not its name.
contrast_cfg <- dea$contrast_results[[dea$default_model]]$get_config()
tests_differences <- contrast_cfg$has_pvalue()
...
<img src="`r knitr::image_uri(system.file("report-assets", "differential-expression.png", package = "prolfquapp"))`" alt="Visual abstract: quantified protein abundances from two sample groups are normalized and statistically compared to produce differential-expression results." style="width: 100%; max-height: 22rem; object-fit: contain;">
...
Input: quantified protein abundances from `r nrow(dea$lfq_data_raw$factors())` samples, analysed with `r report_provenance$software`. The detailed source-data reference is recorded in **Session Info**.

@tbl-parameters lists every processing parameter this analysis was run with. The sections below explain what the individual parameters do.

```{r}
#| label: tbl-parameters
#| tbl-cap: "Processing parameters used for this analysis."
# An html kable renders the decoy/contaminant regexes verbatim; a markdown one
# double-escapes "|" and reads "^" as a superscript.
knitr::kable(
  cfg$processing_options$parameters_table(
    software = report_provenance$software,
    model = report_provenance$model
  ),
  format = "html"
)
```
...
```{r introduction}
#| results: asis
#| eval: !expr "get0('tests_differences', ifnotfound = TRUE)"
cat(r"(
# Analysis background
...
xx <- xx |> dplyr::rename(nr_quantified = ld$hierarchy_keys())
...
The protein matrix is filtered on the minimum number of peptides per protein given in @tbl-parameters. That filter is experiment-wide: a protein is kept only when it is supported by that many distinct peptides across all samples taken together, not per sample. A minimum of 1 means no protein is dropped on peptide count.
...
@fig-nr-per-sample shows the number of quantified proteins per sample, in the protein matrix exported for this analysis — after the peptide filter of @tbl-parameters, and still including the decoys that are dropped before the model fit.

```{r}
#| label: fig-nr-per-sample
#| fig-cap: "Number of quantified proteins per sample; bar height is the protein count per sample. See @tbl-parameters for the peptide filter applied to this matrix."
#| fig-width: 10
#| fig-height: 7
sr$plot_hierarchy_counts_sample(nr_children = 1)
...
pca_widget$width <- 600
pca_widget$height <- 500
sized_widget(pca_widget)
```
...
volcano_widget$width <- 600
volcano_widget$height <- 500
sized_widget(volcano_widget)

```

## Differentially Expressed Proteins

Differentially expressed proteins are selected with the FDR and difference thresholds given in @tbl-parameters. @tbl-nr-significant summarizes the number of significant calls.
...
```{r}
#| results: asis
#| eval: !expr get0("showSignificant", ifnotfound = FALSE)
