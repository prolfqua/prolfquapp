...
- Workunit: `r local_project_conf$workunit_Id %||% "n/a"`
- Project: `r local_project_conf$project_Id %||% "n/a"`
- Order : `r local_project_conf$order_Id %||% "n/a"`
...
The report is organized into tabs. The **Quality Control** tab describes how many `r strings[["targets"]]` were identified, how reproducibly they were detected across samples, how comparable their abundance distributions are, and how variable they are. The **Sample Size Calculation** tab then *consumes* the variability estimated during quality control to size the main experiment for a range of biologically relevant effect sizes. Sample names used throughout are defined in @tbl-sample-mapping.
