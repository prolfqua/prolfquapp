...
```{r introduction, results='asis', eval=!is_saint_report}
cat(r"(
# Introduction
...
```{r prepNrPerSample2, include=FALSE}
sum <- dea$lfq_data_raw$get_Summariser()
...
```{r nrPerSample2, fig.cap="Number of identified proteins across samples.", fig.with=10, fig.height=if (is.null(p_nr_per_sample_2)) 1.2 else 7}
plot_or_empty(
  p_nr_per_sample_2,
...
```{r naHeat, fig.width=7, fig.height=if (is.null(nah)) 1.2 else 6, dpi=300, fig.cap="(ref:naHeat)", fig.alt="", eval=TRUE}
plot_or_empty(
  nah,
...
# Interactive per-sample density curves (ggiraph): hover a curve to highlight it
# and read its sample name. Left panel: empirical abundances; right: normalized.
prolfquapp::intensity_density_ggiraph(
  list(
    "empirical abundance" = dea$lfq_data_raw,
    "normalized abundance" = dea$lfq_data
  ),
  legend = showlegend
)
...
```{r internalCalibration, results = 'asis'}
...
(ref:heatmap) Protein abundance heatmap (rows indicate proteins, columns indicate samples) showing the row scaled $\log_2$ transformed protein abundance value. Co-clustering (hierarchical complete linkage, euclidean distance) of samples and proteins was used. Cell color encodes the row z-score on a green-black-red scale (green: low, black: average, red: high); missing values are shown in light gray.
...
Furthermore, to make it explicit how each estimate was produced, the `estimate_type` column (see table in Figure \@ref(fig:tableAllProt)) records whether a row was fit from `observed` data, rescued via LOD imputation (`lod_imputed`), or filled from a group-mean fallback (`missing_fallback`); rescued/fallback rows are visualized with a distinct colour in Figure \@ref(fig:volcanoplot). The `modelName` column records the selected analysis method (the facade key, e.g. `lm`, `rfit`, `limma`).
...
(ref:volcanoplot)  Interactive volcano plot showing $-\log_{10}$ transformed FDR as function of the difference between groups (hover a point for its identifier and statistics). The red line indicates the $-log_{10}(FDR)$ of FDR = `r cfg$processing_options$FDR_threshold`, while the green lines represent the difference of minus and plus `r cfg$processing_options$diff_threshold`. Points are coloured by `estimate_type`, so rows estimated by missing-value imputation or group-mean fallback are visually distinguished from observed fits. Contaminants are drawn as triangles (and flagged in the tooltip); decoys carry no statistics and are omitted.


```{r volcanoplot, fig.cap = "(ref:volcanoplot)",fig.alt="", fig.width=9, fig.height=7, include = TRUE, eval = TRUE}

# Interactive volcano (ggiraph). Coloured by estimate_type; contaminants (the
# annotation CON flag) are drawn as triangles and flagged in the tooltip, so they
# are kept and visible but distinguishable. Decoys carry NA statistics and are
# therefore dropped automatically (never plotted). Built from annotated_contrasts
# directly so it does not depend on other chunks' state.
vdata <- as.data.frame(dea$annotated_contrasts) |>
  tidyr::unite("ID", tidyselect::all_of(hkey), sep = "~", remove = FALSE)

prolfquapp::volcano_ggiraph(
  vdata,
  effect = effect_col,
  score = score_col,
  ...
  score_threshold = cfg$processing_options$FDR_threshold
)
...
Here we use the FDR threshold of `r cfg$processing_options$FDR_threshold` and a difference threshold of `r cfg$processing_options$diff_threshold` to select differentially expressed proteins. Table \@ref(tab:nrsignificant) summarizes the number of significant calls.
...
*This report was generated from the R Markdown template `Grp2Analysis_V2_R6.Rmd` included in the `prolfquapp` R package (version `r packageVersion("prolfquapp")`).*
