...
if (show_text) {
  ...
  n_samples <- length(unique(lfqdata$data_long()[[lfqdata$get_config()$sample_name]]))
  if (n_samples < 2) {
...
ggplot2::ggplot(sampleSize2, ggplot2::aes(x = probs, y = N)) +
  ...
  ggplot2::facet_wrap(as.formula(paste0("~ ", "delta + " , paste0( lfqdata$get_config()$factor_keys_depth(), collapse = " + " ))))
   
tmp <- sampleSize2 |> tidyr::pivot_wider(id_cols = c("probs","sdtrimmed", lfqdata$get_config()$factor_keys_depth()), names_from =  delta, values_from = N  )
