...
```{r results='asis'}
if (isSig) {cat("The protein abundance heatmap (Figure \\@ref(fig:heatsig)) is pre-filtered for prays that show _BFDR_ < ", SEP$FDRthreshold," and EFC score > ", SEP$FCthreshold, ".")
} else {
  cat("There are not significant results given the _BFDR_ < ", SEP$FDRthreshold," and EFC score > ", SEP$FCthreshold, ".")
}

```

(ref:heatsig) Heatmap showing the $\log_2$ transformed protein abundance estimate for prays with _BFDR_ below `r SEP$FCthreshold` for at least one prey.
...
```{r DT,  eval = isSig}
cap <- paste0("Proteins with BFDR < ", SEP$FDRthreshold, " and EFC score > ", SEP$FCthreshold, ".")
...
bb + ggplot2::theme_classic()

```


(ref:vennDiagram) Venn diagram showing the number of prays common or distinct to the bait proteins in the data set, for _BFDR_ < `r SEP$FDRthreshold` and _EFC_ score < `r SEP$FCthreshold`.
