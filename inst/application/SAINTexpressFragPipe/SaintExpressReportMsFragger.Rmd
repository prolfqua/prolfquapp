...
In total `r SEP$lfqdata$hierarchy_counts()[2]` proteins (min. 2 peptides per protein) were identified and quantified using `r sp_string` across all samples (we will denote this as the protein abundance estimate from here on). Bar plot below shows the number of identified proteins per raw file.
...
(ref:vennProteins) Venn diagram showing the number of proteins present in all possible intersections. 
...
(ref:volcanoPlotBfdr) Volcano Plot showing $-\log_{10}$ transformed BFDR as function of $\log_2$ transformed effect size. The blue line shows the $-log_{10}$ transformed BFDR threshold of `r SEP$FDRthreshold`, while the red line the $\log_2$ transformed FC of `r SEP$FCthreshold`. 




```{r setupVolcano, echo = FALSE}
datax <- SEP$cse$get_contrasts()
...
xd <- prolfqua::volcano_Plotly( datax , effect = "log2_EFCs", significance = "BFDR", contrast = "Bait", xintercept =  log2(SEP$FCthreshold), yintercept = SEP$FDRthreshold)
plotly::subplot(xd, shareX = TRUE, shareY = TRUE)
...
The Bait Prey interaction candidate lists are filtered using an Bayesian false discovery rate (BFDR) threshold of `r SEP$BFDRthreshold` and an  empirical fold-change score (EFC) threshold of `r SEP$FCthreshold` which corresponds to a $\log_2(EFC)$ of `r log2(SEP$FCthreshold)`.
...
all <- SEP$cse$get_contrasts() |> dplyr::group_by(Bait) |> dplyr::summarise(all = dplyr::n())
...
sig <- dplyr::select(SEP$sig, Bait, Prey, log2_EFCs, BFDR, SaintScore)
DT::datatable(sig)
```
...
- presenceAbsence - spreadsheet containing the presence absence information used to create the VennDiagram figure \@ref(fig:vennProteins). Cells with 0 entries indicate protein absence while 1 entries protein presence.
...
- `all_<baitName>.txt` all prey proteins.
- `accepted_<baitName>.txt` prey proteins accepted with the BFDR and EFC specified.


We do also include the file: which can be imported into 
