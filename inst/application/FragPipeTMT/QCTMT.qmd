...
pathtoZip <- if (exists("params") && !is.null(params$zipfile)) {
  params$zipfile } else { "qcdata/FragPipeTMT_o33075_HumanBovine.zip"  }
...
Below are the amino acid frequencies .
...
#debug(prolfqua::tidy_FragPipe_psm)
...
nrKmod <- nrKmod |> dplyr::group_by(modSeq , n , n1) |> dplyr::summarize(nrKmod = dplyr::n(), .groups="drop") |> dplyr::ungroup()

```

- Total number of Lysine residues: `r (nrK$nrK * nrK$n1) |> sum() `
- Number of modified Lysine residues : `r (nrKmod$nrKmod * nrKmod$n1) |> sum()`
- Percent modified Lysine residues: `r round( ( (nrKmod$nrKmod * nrKmod$n1) |> sum()) / ((nrK$nrK * nrK$n1) |> sum()) * 100)` \%
