...
allmods <- lfqdata$data |> select(mod_peptide_Id, Spectrum) |> distinct()
allmods <- allmods |> dplyr::group_by(mod_peptide_Id) |> summarize(n = n())
allmods <- allmods |> separate(mod_peptide_Id, into = c("modSeq", "Assigned.Modifications"), sep = "~")
allmods <- allmods |> tidyr::separate_longer_delim("Assigned.Modifications", delim = ",")
allmods <- allmods |> mutate(Assigned.Modifications = trimws(Assigned.Modifications))
allmods <- allmods |> mutate(Modification = gsub("^[0-9]+","", Assigned.Modifications))
...
```{r }
allmods$n1 <- 1
xA <- allmods |> select(modSeq, n, n1) |> distinct()
xN <- allmods |> filter(Modification == "N-term(229.1629)")
xN <- xN |> select(modSeq, n, n1) |> distinct()
...
```{r}
xL <- allmods |> filter(Modification == "K(229.1629)")
xN <- allmods |> dplyr::select(.data$modSeq, n, n1) |> dplyr::distinct()
xN <- xN |> filter(grepl("K", modSeq))
...
xx <- lfqdata$data
totalPerChannel <- xx |> group_by(!!sym(lfqdata$config$table$sampleName) )|> summarize(totalAbundance = sum(!!sym(lfqdata$response()), na.rm=TRUE))

totalPerChannel |> ggplot(aes(x = sampleName, y = totalAbundance)) + 
...
nudgeval = 0.1
p126 <- totalPerChannel |> filter(grepl("126",sampleName) )
totalPerChannel$norm126abundance <- totalPerChannel$totalAbundance / p126$totalAbundance
totalPerChannel |> ggplot(aes(x = sampleName, y = norm126abundance)) + 
  geom_bar(stat = "identity", position = "dodge", colour = "black", fill = "white") + 
  ggplot2::geom_text(aes(label = round(.data$norm126abundance,2)), nudge_y = nudgeval, angle = 65) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
```{r}
modSeqD <- allmods |> select(modSeq, n) |> distinct() 
