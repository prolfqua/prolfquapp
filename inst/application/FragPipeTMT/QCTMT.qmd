...
pathtoZip <- if (exists("params") && !is.null(params$zipfile)) {
  params$zipfile } else { "qcdata/FragPipeTMT-datasetvsUP000005640.zip"  }
...
addinfo <- data.frame(name = c("workunit_id", "project_id", "order_id", "datasetname", "fastasequence"),
                      value =  c(
                        ifelse(is.null(pconfig$job_configuration$workunit_id),"",pconfig$job_configuration$project_id),
