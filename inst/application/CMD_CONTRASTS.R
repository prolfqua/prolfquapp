...
option_list <- list(
  optparse::make_option(c("--control"),
...
if (!one_factor && !two_factor) {
  logger::log_error("Specify --control (scenario 1) or --f1 + --f2 (scenario 2).")
  optparse::print_help(parser)
  quit(status = 1)
}
...
if (one_factor) {
  ...
  annot     <- res$annot
  group_col <- if (!is.null(opt$group)) opt$group else res$atable$factors[["G_"]]

  logger::log_info("Group column : ", group_col)
  logger::log_info("Levels found : ", paste(unique(annot[[group_col]]), collapse = ", "))

  if (!opt$control %in% annot[[group_col]]) {
    logger::log_error("--control '", opt$control, "' not found in '", group_col, "'.")
    quit(status = 1)
  }
  ...
  logger::log_info("CONTROL column added (C = '", opt$control, "', T = everything else):\n",
    paste(capture.output(print(table(annot_out[[group_col]], annot_out$CONTROL))), collapse = "\n"))
}
...
if (two_factor) {
  ...
  if (length(missing_cols) > 0) {
    logger::log_error("Column(s) not found: ", paste(missing_cols, collapse = ", "))
    quit(status = 1)
  }

  logger::log_info("f1 = '", opt$f1, "' levels: ", paste(unique(df[[opt$f1]]), collapse = ", "))
  logger::log_info("f2 = '", opt$f2, "' levels: ", paste(unique(df[[opt$f2]]), collapse = ", "))

  res       <- prolfqua::annotation_add_contrasts(df,
    primary_col   = opt$f1,
...
if (two_factor) {
  ct <- dplyr::distinct(annot_out[!is.na(annot_out$ContrastName), c("ContrastName", "Contrast")])
  logger::log_info("Suggested contrasts:\n",
