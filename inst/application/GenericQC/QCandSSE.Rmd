...
target_mapping <- list(
  ...
  "protein" = list("peptide" = "protein", "protein" = "proteins", "proteins and peptides" = "proteins"),
)

strings <- target_mapping[[params$target_type]]
```
...
This document summarizes the `r strings[["peptide"]]` variability to asses the reproducibility of the biological samples and estimates the sample sizes needed for the main experiment. 
...
Here we summarize the number of `r strings[["peptide"]]` measured in the QC experiment.
`r typical_observations`
While the overall number of `r strings[["proteins and peptides"]]` can highly vary depending of the type of experiment, it is crucial that the number of `r strings[["proteins and peptides"]]`  between your biological replicates is similar (reproducibility).
...
prolfqua::table_facade( 
  data.frame(NR = x),
  caption = paste0("Nr of ", strings[["proteins and peptides"]], " detected in all samples."))
...
(ref:hierarchyCountsSampleBarplot) Number of quantified `r strings[["peptides"]]` per sample.
...
```{r upsetMissing, fig.width=9, fig.height=9, fig.cap= "The plot shows the relationships between sets of proteins through their intersections, as well as the size of each set. The elements that are present in each intersection are shown as circles or dots in the matrix, and the size of each set is represented by the height of the corresponding row."}
...
Ideally, we identify each `r strings[["peptide"]]` in all of the samples. However, because of the limit of detection (LOD) low-intensity `r strings[["peptides"]]` might not be observed in all samples. Ideally, the LOD should be the only source of missingness in biological replicates. The following figures help us to verify the reproducibility of the measurement at the level of missing data.

```{r}
height <- max(7, nrSamples * 0.1)
```

(ref:missingFigIntensityHistorgram) Top - intensity distribution of `r strings[["peptides"]]` with 0, 1 etc. missing values. B - number of `r strings[["peptides"]]` with 0, 1, 2 etc. missing value.
...
(ref:missingnessHeatmap) Heatmap of missing `r strings[["peptide"]]` quantifications clustered by sample.
...
cat("Without applying any intensity scaling and data preprocessing, the ", strings[["peptide"]], " intensities in all samples should be similar. To assess this we plotted the distribution of the ", strings[["peptide"]], " intensities in the samples (Figure \\@ref(fig:plotDistributions)) as well as the distribution of the coefficient of variation CV for all ", strings[["peptides"]], " in the samples (Figure \\@ref(fig:intensityDistribution)). Table \\@ref(tab:printTable) summarises the CV.")
...
(ref:plotTransformedIntensityDistributions) `r strings[["peptide"]]` intensity distribution after transformation.
...
(ref:correlationHeat) Heatmap of `r strings[["peptide"]]` intensity correlation between samples.
...
(ref:sdviolinplots) Visualization of `r strings[["peptide"]]` standard deviations. A) all. B) - for low (bottom 50) and high intensity (top 50).
...
(ref:sdecdf) Visualization of `r strings[["peptide"]]` standard deviations as empirical cumulative distribution function. A) all. B) - for low (bottom 50) and high intensity (top 50).
...
(ref:overviewHeat) Sample and `r strings[["peptide"]]` Heatmap.
...
Table \@ref(tab:sampleSize) and Figure \@ref(fig:figSampleSize) summarizes how many samples are needed to detect a fold change of $0.5, 1, 2$ at a confidence level of $95\%$ and power of $80\%$, for $50, 60, 70, 80$ and $90\%$ percent of the measured `r strings[["peptides"]]`.

(ref:figSampleSize) Graphical representation of the sample size needed to detect a log fold change greater than delta with a significance level of $0.05$ and power 0.8 when using a t-test to compare means, in $X\%$ of `r strings[["peptides"]]` (x - axis).
...
caption <- paste("Number of quantified ", strings[["proteins and peptides"]], " per sample.")
