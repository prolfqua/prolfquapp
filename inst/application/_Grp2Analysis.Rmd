...
datax <- GRP2$RES$contrastsData
```


```{r tableAllProt, fig.cap="Differential expression analysis results of all proteins."}
bb <- datax |> dplyr::select(all_of(c("protein_Id" ,  "description", "contrast", "modelName", "diff", "FDR" )))
...
ctdata <- datax_signif |> dplyr::select(all_of(c("protein_Id" ,  "description", "contrast", "modelName", "diff", "FDR"  )))
...
if(sifnif$hierarchy_counts()[1] > 30){
  rownames = FALSE
} else {
  rownames = TRUE
}
...
- __missing_information__ - spreadsheet containing information in how many samples of a group a protein is observed.
