...
tab <- data.frame(table(dea$lfq_data$factors()[dea$lfq_data$config$factor_keys_depth()]))
...
fx <- dplyr::left_join(fx,xx, by = ld$config$sampleName)
fx <- dplyr::left_join(fx,x2, by = ld$config$sampleName)


knitr::kable(fx, caption = "LC-MS samples annotation table. The content of the sampleName column is used as a short form to plot labels. The group to which a sample is assigned is shown in the column group.")
...
sum <- dea$lfq_data$get_Summariser()
...
```{r countProtWithNAs}
res <- dea$lfq_data$to_wide(as.matrix = TRUE)$data
res[!is.na(res)] <- 0
res[is.na(res)] <- 1
allrows <- nrow(res)
...
pups <- prolfqua::UpSet_interaction_missing_stats(dea$lfq_data$data, dea$lfq_data$config, tr = 1)
...
showlegend <- if (nrow(dea$lfq_data$factors()) > 12) {FALSE} else {TRUE}
p1 <- pl$intensity_distribution_density(legend = showlegend)

plTransformed <- dea$lfq_data_transformed$get_Plotter()
...
if (length(internal_ids) > 0 && !all(is.na(internal_ids)) && !all(internal_ids == "")) {
  ...
  cat("After transformation, the per-sample intensity of each reference protein was subtracted from all proteins in that sample (`center_to_reference`). ")
...
pl <- dea$lfq_data_transformed$get_Plotter()
ph <- pl$heatmap()
...
dea$lfq_data_transformed$get_Plotter()$pca_plotly()
```
...
library(dplyr)
mx <- datax |> dplyr::mutate(passes = abs(diff) >  cfg$processing_options$diff_threshold  & FDR < cfg$processing_options$FDR_threshold)
...
signif <- dea$lfq_data_transformed$get_copy()
...
*This report was generated from the R Markdown template `_Grp2Analysis_V2_R6.Rmd` included in the `prolfquapp` R package (version `r packageVersion("prolfquapp")`).*
