...
/* optional: reduce side margins */
body {
  margin-left: 2% !important;
  margin-right: 2% !important;
  padding-top: 5px !important;
}
/* aggressive title spacing reduction - single line */
#header {
...
knitr::kable(fx, caption = paste0(
  ...
  "is assigned to is shown in the column group."))
...
if (grp2$processing_options$transform == "robscale") {
  cat(
    "To do this the z-score of the $\\log_2$ transformed metabolite",
    "abundances are computed.",
    "Because we need to estimate the metabolite differences on the",
  ...
  cat(
    ...
    "the data, e.g. in case of targeted metabolomics experiments,",
...
```{r}
hkey <- grp2$RES$lfqData$config$hierarchy_keys()
```
...
mx <- datax |>
  dplyr::mutate(
    passes = abs(diff) > grp2$processing_options$diff_threshold &
      FDR < grp2$processing_options$FDR_threshold)
x <- mx |>
...
cat(paste(
  ...
  "of log2 transformed metabolite abundances of all",
  "significant calls."))
