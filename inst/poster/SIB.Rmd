...
The entire working directory is archived. It contains all the data and R code and data to <b>replicate<b/> the analysis. 
...
```{r configyml, out.width="70%", include = TRUE}
...
If subject is specified then the model is `abundance ~ group + subject`, otherwise
 `abundance ~ group`. The group differences to compute are determined from the group and control columns.
...
- Archived directory contains all information needed to replicate analysis <br/>
rerun the analysis on your PC
- Our users know Excel and like XLSX files
- Shiny app in development


```{r htmlreport, out.width="90%"}
...
```{r url, out.width="60%",fig.show='hold', fig.align="center"}
#knitr::include_graphics(c("qrcodeProlfqua.png", "qrCodeProlfquApp.png"))

img1 <-  grid::rasterGrob(as.raster(png::readPNG("qrcodeProlfqua.png")), interpolate = FALSE)
img2 <-  grid::rasterGrob(as.raster(png::readPNG("qrCodeProlfquApp.png")), interpolate = FALSE)
gridExtra::grid.arrange(img1, img2, ncol = 2)
```


```{r sib, out.width="30%", fig.align="center"}
