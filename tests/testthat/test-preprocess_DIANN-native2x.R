...
test_that("diann_read_output derives identical raw.file from native `Run` and renamed `File.Name`", {
  ...
  native <- prolfquapp::diann_read_output(make_report_rows(runs, run_col = "Run"))
  # legacy shim: `Run` renamed to `File.Name`, value is still the bare basename
  renamed <- prolfquapp::diann_read_output(make_report_rows(runs, run_col = "File.Name"))
...
test_that("diann_read_output falls back to DIA-NN 1.x `File.Name` full paths", {
  ...
  out <- prolfquapp::diann_read_output(make_report_rows(file_names, run_col = "File.Name"))
  expect_identical(
    out$raw.file,
    c(
      "20260623_010_C42222_S1172811_Plate_7001_H1",
...
test_that("get_DIANN_files discovers and prefers the native parquet", {
  ...
  writeLines(c(">sp|P11111|PROT1_TEST one", "PEPTIDEK"), file.path(dir, "database.fasta"))
...
test_that("preprocess_DIANN builds an LFQData from a native DIA-NN 2.x parquet", {
  ...
  rows <- do.call(rbind, lapply(runs, function(run) {
    do.call(rbind, lapply(proteins, function(pg) {
      data.frame(
        Run = run,
