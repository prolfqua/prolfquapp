...
test_that("DiffExpQC Quarto report declares report provenance metadata", {
  ...
  qmd <- paste(
    readLines(file.path(report_source_dir, "DiffExpQC_R6_tabset.qmd"), warn = FALSE),
    collapse = "\n"
  )

  expect_match(qmd, "Report Information", fixed = TRUE)
  ...
  expect_match(qmd, "Generated by", fixed = TRUE)
