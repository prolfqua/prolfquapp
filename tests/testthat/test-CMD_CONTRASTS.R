...
csv_s1   <- system.file("application/contrasts/scenario1_single_factor.csv", package = "prolfquapp")
csv_s2   <- system.file("application/contrasts/scenario2_two_factor.csv",   package = "prolfquapp")
...
test_that("scenario 1 - single factor: writes output with ContrastName/Contrast columns", {
  ...
  status <- system2("Rscript", c(script, csv_s1, "--control", "WT", "-o", out),
...
test_that("scenario 2 - two factor: writes output with ContrastName/Contrast columns", {
  ...
  status <- system2("Rscript", c(script, csv_s2, "--f1", "treatment", "--f2", "time", "-o", out),
    stdout = TRUE, stderr = TRUE)
  ...
  ct <- dplyr::distinct(result[!is.na(result$ContrastName), c("ContrastName", "Contrast")])
  expect_gt(nrow(ct), 0)
