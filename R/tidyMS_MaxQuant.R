...
tidyMQ_ProteinGroups <- function(MQProteinGroups) {
  if (is.character(MQProteinGroups)) {
    if (grepl("\\.zip$", tolower(MQProteinGroups))) {
      ...
      MQProteinGroups <- read.csv(
        unz(MQProteinGroups, "proteinGroups.txt"),
        header = TRUE,
        sep = "\t",
  ...
  pintLFQ <- dplyr::select(
    MQProteinGroups,
    "protein.group.id" = "id",
    starts_with("lfq.intensity.")
  )
  pintMSCount <- dplyr::select(
    MQProteinGroups,
    "protein.group.id" = "id",
  ...
  meta <- meta |>
    dplyr::mutate(proteinID = gsub(";.*$", "", .data$majority.protein.ids))
  pint <- pint |>
    tidyr::gather(
      key = "raw.file",
      value = "mq.protein.intensity",
      starts_with("intensity.")
    ) |>
  ...
  pintLFQ <- pintLFQ |>
    tidyr::gather(
      key = "raw.file",
      value = "mq.protein.lfq.intensity",
      starts_with("lfq.intensity.")
    ) |>
  ...
  pint <- dplyr::inner_join(
    pint,
    pintLFQ,
    by = c("protein.group.id", "raw.file")
  )
  pint <- dplyr::inner_join(
    pint,
    pintCount,
...
tidyMQ_Peptides <- function(MQPeptides, proteotypic_only = TRUE) {
  if (is.character(MQPeptides)) {
    if (length(MQPeptides) == 0) {
      logger::log_error(
        "No peptides.txt file found. MaxQuant preprocessor requires peptides.txt in the input directory."
      )
      stop("peptides.txt not found.")
    }
    if (grepl("\\.zip$", tolower(MQPeptides))) {
      MQPeptides <- read.csv(
        unz(MQPeptides, "peptides.txt"),
        header = TRUE,
        sep = "\t",
        stringsAsFactors = FALSE
      )
  ...
  meta <- dplyr::select(
    ...
    dplyr::mutate(
      !!"unique.groups" := dplyr::case_when(
        !!sym("unique.groups") == "yes" ~ TRUE,
        !!sym("unique.groups") == "no" ~ FALSE
      )
    ) |>
    dplyr::mutate(
      !!"reverse" := dplyr::case_when(
        !!sym("reverse") == "+" ~ TRUE,
        !!sym("reverse") == "" ~ FALSE
      )
    )
  ...
  if (ncol(idtype) > 1) {
    ...
    PepIntensities <- dplyr::inner_join(
      PepIntensities,
      PepIDType,
      by = c("peptide.id", "raw.file")
    )
  } else {
...
preprocess_MQ_peptide <- function(
  ...
  peptide <- prolfquapp::tidyMQ_Peptides(
    quant_data,
    proteotypic_only = proteotypic_only
  )
  nrPeptides_exp <- peptide |>
    dplyr::select(leading.razor.protein, sequence) |>
    dplyr::distinct() |>
  ...
  fasta_annot <- dplyr::left_join(
    nrPeptides_exp,
    fasta_annot,
    by = c("leading.razor.protein" = "fasta.id")
  )
