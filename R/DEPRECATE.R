...
generate_DEA_reports <- function(lfqdata, GRP2, prot_annot) {
  ...
  if (!is.null(GRP2$pop$Contrasts)) {
    ...
    lfqwork$data <- lfqdata$data |>
      dplyr::filter(.data$Group_ %in% levels$Group_)
      ...
      for (j in 1:nrow(levels)) {
        if (i != j) {
          if (levels$control[j] == "C") {
            ...
            Names <- c(
              Names,
              paste0(levels$Group_[i], "_vs_", levels$Group_[j])
            )
          }
        }
      }
    ...
    for (i in seq_along(levels$Group_)) {
      for (j in seq_along(levels$Group_)) {
        if (i != j) {
          ...
          names(GRP2$pop$Contrasts) <- paste0(
            levels$Group_[i],
            "_vs_",
            levels$Group_[j]
          )
          ...
          lfqwork$data <- lfqdata$data |>
            dplyr::filter(
              .data$Group_ == levels$Group_[i] |
                .data$Group_ == levels$Group_[j]
            )
          grp2 <- prolfquapp::make_DEA_report(lfqwork, prot_annot, GRP2)
...
#' @family workflow
#'
make_DEA_report <- function(lfqdata, protAnnot, GRP2) {
  ...
  GRP2$RES$Summary <- data.frame(
    ...
    percentOfContaminants = round(
      protAnnot$annotate_contaminants() / allProt * 100,
      digits = 2
    ),
    percentOfFalsePositives = round(
      protAnnot$annotate_decoys() / allProt * 100,
      digits = 2
    ),
  ...
  if (GRP2$pop$removeCon || GRP2$pop$removeDecoys) {
    lfqdata <- lfqdata$get_subset(protAnnot$clean(
      contaminants = GRP2$pop$removeCon,
      decoys = GRP2$pop$removeDecoys
    ))
  ...
  ################## Run Modelling ###############
  factors <- transformed$config$factor_keys()[
    !grepl("^control", transformed$config$factor_keys(), ignore.case = TRUE)
  ]

  if (is.null(GRP2$pop$interaction) || !GRP2$pop$interaction) {
    formula <- paste0(
      transformed$config$get_response(),
      " ~ ",
  ...
  if (is.null(GRP2$pop$model_missing) || GRP2$pop$missing) {
  ...
  datax <- datax |>
    dplyr::filter(
      .data$FDR < GRP2$pop$FDRthreshold &
        abs(.data$diff) > GRP2$pop$Diffthreshold
    )
...
sanitize_grouping_var <- function(annot) {
  ...
  annot[[groupingVAR]] <- gsub(
    "\\.",
    "_",
    gsub("\\.\\.", "\\.", make.names(annot[[groupingVAR]]))
  ) # sanitize group variable
  return(annot)
}
...
read_yaml_deprec <- function(ymlfile, application = "FragPipeTMT") {
  ...
  idxzip <- grep(
    "[0-9]{7,7}.zip|DIANN_Result_WU[0-9]{6,6}.zip",
    yml$application$input[[1]],
    ignore.case = TRUE
  )

  GRP2$project_spec$input_Id <- yml$job_configuration$input[[1]][[
    idxzip
  ]]$resource_id
  GRP2$project_spec$input_URL <- yml$job_configuration$input[[1]][[
    idxzip
  ]]$resource_url
  ...
  GRP2$pop$Diffthreshold <- as.numeric(
    yml$application$parameters$`4|Difference_threshold`
  )
  GRP2$pop$FDRthreshold <- as.numeric(
    yml$application$parameters$`5|FDR_threshold`
  )

  GRP2$pop$removeCon <- if (
    yml$application$parameters$`6|remConDec` == "true"
  ) {
    TRUE
  } else {
    FALSE
  }
  GRP2$pop$removeDecoys <- if (
    yml$application$parameters$`6|remConDec` == "true"
  ) {
    TRUE
  } else {
...
#' nrow(report2)
#' }
diann_read_output_deprec <- function(
  data,
  Lib.PG.Q.Value = 0.01,
  PG.Q.Value = 0.05
) {
