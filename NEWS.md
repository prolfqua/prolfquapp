...
- Removed the unused exports `exp2()`, `copy_docker_script()`, `make_annotated_experiment()`, `get_FP_multiSite_files()`, `tidy_FragPipe_psm_V2()` and `tidy_FragPipe_MSstats_csv()`; nothing in prolfquapp or its downstream packages called them. The rest of the package code was simplified, about 30% fewer lines, without changing exported functions or the written artifacts.
